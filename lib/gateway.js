...
function getGatewayConfig() {
  let url = process.env.KNOWSPACE_GATEWAY_URL || process.env.CLAWDBOT_GATEWAY_URL || '';
  let token = process.env.KNOWSPACE_GATEWAY_TOKEN || process.env.CLAWDBOT_GATEWAY_TOKEN || '';
...
function createEventClient() {
  ...
  function handleClose(code) {
    ...
    if (code === 1008) {
      const identity = getDeviceIdentity();
      console.error(`[knowspace] Device auth rejected. Approve this device on the gateway: ${identity.deviceId}`);
    }
