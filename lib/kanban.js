...
function parseKanban(content) {
  ...
  for (const line of lines) {
    ...
    } else if (currentLane && !currentCard && line.trim().startsWith('<!--')) {
      // Lane-level meta lives between the ## header and the first card.
      // Recognized keys: ks:lane-assignee, ks:lane-prompt, ks:lane-auto-dispatch.
      // We use a dedicated regex (not parseKsLine) because ks:lane-prompt
      // values frequently contain spaces, which the generic parser
      // interprets as the multi-attr form.
      const m = line.match(/^\s*<!--\s*ks:lane-([a-z][a-z0-9-]*)=(.*?)\s*-->\s*$/);
      if (m) {
        const sub = m[1];
        const val = m[2];
        if (sub === 'assignee')      currentLane.meta.assignee = val;
        else if (sub === 'prompt')   currentLane.meta.prompt = val.replace(/\\n/g, '\n');
        else if (sub === 'auto-dispatch') currentLane.meta.autoDispatch = val === 'true';
      }
...
function serializeLaneMeta(lane) {
  const out = [];
  if (lane.meta?.assignee) out.push(`<!-- ks:lane-assignee=${lane.meta.assignee} -->`);
  if (lane.meta?.autoDispatch) out.push(`<!-- ks:lane-auto-dispatch=true -->`);
  if (lane.meta?.prompt) {
    const flat = String(lane.meta.prompt).replace(/\n/g, '\\n');
    out.push(`<!-- ks:lane-prompt=${flat} -->`);
  }
...
module.exports = { parseKanban, serializeKanban, serializeLaneMeta, parseKsLine, extractMetaFromBody };
